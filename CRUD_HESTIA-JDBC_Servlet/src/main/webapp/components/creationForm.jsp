...
<%
    ...
    String[] regex = {"^([A-Z]\\p{L}+|[A-Z]\\p{L}+(\\s[A-Z]\\p{L}+)+)$", "^\\S{8,100}@germinare\\.org\\.br$", "^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[^A-Za-z0-9])\\S{4,100}$", "^[1-9][0-9]{1,6},\\d{2}$", "^\\S+$", "^([1-9][0-9]{1,10}|[1-9]).\\d{0,2}$", "^([1-9][0-9]{1,3}|[1-9]).\\d{0,2}$", "^((\\S[^[A-Za-z]]*|[A-Z]\\p{L}*)|(\\S[^[A-Za-z]]*|[A-Z]\\p{L}*)(\\s(\\S[^[A-Za-z]]+|\\p{L}*))*)$"};
%>
